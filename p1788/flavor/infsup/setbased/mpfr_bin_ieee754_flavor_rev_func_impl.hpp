...
template<typename T>
int mpfr_bin_ieee754_flavor<T>::pow_rev_inf(mpfr_var& xl,
                                            mpfr_var const& cu,
                                            int p)
{
    int t1;
    if (p != -p)
    {
        t1 = -mpfr_root(xl(), const_cast<mpfr_var&>(cu)(), -p, MPFR_RNDU);
    }
    else
    {
        int t2 = -mpfr_sqrt(xl(), const_cast<mpfr_var&>(cu)(), MPFR_RNDU);
	t1 = -mpfr_root(xl(), xl(), -(p >> 1), MPFR_RNDU);
	if (t1 == 0) t1 = t2;
    }
    int t = mpfr_ui_div(xl(), 1, xl(), MPFR_RNDD);
    if (t == 0) t = t1;
    if (p == -1 || t == 0) return t;
    mpfr_var cm;
    for (;;)
    {
        mpfr_nextabove(xl());
        xl.subnormalize(0, MPFR_RNDU);
        if (!mpfr_regular_p(xl())) break;
        t1 = mpfr_pow_si(cm(), xl(), p, MPFR_RNDU);
        t = mpfr_cmp(const_cast<mpfr_var&>(cu)(), cm());
        if (t == 0) t = t1;
        if (t == 0) return 0;
        if (t > 0) break;
    }
    mpfr_nextbelow(xl());
    xl.subnormalize(0, MPFR_RNDD);
    return -1;
}

template<typename T>
int mpfr_bin_ieee754_flavor<T>::pow_rev_sup(mpfr_var& xu,
                                            mpfr_var const& cl,
                                            int p)
{
    int t1;
    if (p != -p)
    {
        t1 = -mpfr_root(xu(), const_cast<mpfr_var&>(cl)(), -p, MPFR_RNDD);
    }
    else
    {
        int t2 = -mpfr_sqrt(xu(), const_cast<mpfr_var&>(cl)(), MPFR_RNDD);
	t1 = -mpfr_root(xu(), xu(), -(p >> 1), MPFR_RNDD);
	if (t1 == 0) t1 = t2;
    }
    int t = mpfr_ui_div(xu(), 1, xu(), MPFR_RNDU);
    if (t == 0) t = t1;
    if (p == -1 || t == 0) return t;
    mpfr_var cm;
    for (;;)
    {
        mpfr_nextbelow(xu());
        xu.subnormalize(0, MPFR_RNDD);
        if (!mpfr_regular_p(xu())) break;
        t1 = mpfr_pow_si(cm(), xu(), p, MPFR_RNDD);
        t = mpfr_cmp(const_cast<mpfr_var&>(cl)(), cm());
        if (t == 0) t = t1;
        if (t == 0) return 0;
        if (t < 0) break;
    }
    mpfr_nextabove(xu());
    xu.subnormalize(0, MPFR_RNDU);
    return 1;
}
                                     ...
                                     int p)
{
    ...
    if (p % 2 == 0)
    {
        if (p < 0)  // negative even
        {
            ...
            mpfr_var xu;
            int t_u = pow_rev_sup(xu, cl, p);

            mpfr_var xl;
            int t_l = pow_rev_inf(xl, cu, p);
